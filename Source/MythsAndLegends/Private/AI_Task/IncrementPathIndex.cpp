...
EBTNodeResult::Type UIncrementPathIndex::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
    ...
    {
        int32 CurrentIndex = NPC_Con->GetBlackboardComponent()->GetValueAsInt(NPC_Con->Key_TargetIndex);
        ...
        if(auto* const NPC_Char = Cast<ANPC>(NPC_Con->GetPawn()))
        {
            ...
            NPC_Con->GetBlackboardComponent()->SetValueAsInt(NPC_Con->Key_TargetIndex, CurrentIndex);
